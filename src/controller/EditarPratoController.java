...
public class EditarPratoController extends Tela implements Initializable {
	...
	private GerenciaCardapio gerenciaCardapio = new GerenciaCardapio();
	...
	private Prato prato = (Prato) ObjetoSelecionado.getInstance().getObj();
    ...
    void editar(ActionEvent event) {
    	if (!nomeTextField.getText().isBlank() && !precoTextField.getText().isBlank()
			...
			try {
				gerenciaCardapio.editarPrato(cardapio, listaProdutos, prato.getId(), info);
			} catch (PrecoInvalido | QuantidadeInvalida | ProdutoNaoCadastrado | FormatoIngredientesInvalido | ErroGrave
					| PratoNaoCadastrado e) {
				e.printStackTrace();
			}
    ...
    private void carregarIngredientes() {
    	...
    	for (String produto: prato.getReceita().keySet()) {
    		String quantidade = prato.getReceita().get(produto).toString();
    		String medida;
    		try {
    			medida = prato.getProdutos().get(produto).get(0).getUnidadeDeMedida();
    		} catch (java.lang.IndexOutOfBoundsException e) {
    			medida = "un.";
    		}
    		Ingrediente ingrediente = new Ingrediente(produto, medida, quantidade);
    		obsIngredientes.add(ingrediente);
    		
    		obsProdutos.remove(produto);
    	}
    ...
    private void carregarDados() {
    	...
    	precoTextField.setText(prato.getPreco().toString());
    	descricaoTextField.setText(prato.getDescricao());
    	categoriaComboBox.setValue(prato.getCategoria());
    }
